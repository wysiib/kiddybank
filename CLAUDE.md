...
**Frontend** — no JS build step. Routes use German paths (`/ueberweisen`, `/eltern`, `/konto/{id}`, `/festgeld/.../abholen`). `app/static/app.js` is the amount stepper and PIN keypad (kids tap buttons instead of typing); HTMX is vendored and used for partials like `_preview.html`. `sw.js` is served from `/sw.js` (root scope): cache-first static, network-first pages, never caches POSTs, and wipes cached pages on logout so a shared tablet can't leak balances. Bump the cache version constants there if you change caching behavior. `sw.js` caches `/static/` cache-first, so bump `ASSETS` whenever `app.js` or `app.css` change in a way installed clients must pick up (its `activate` handler deletes older cache names).
