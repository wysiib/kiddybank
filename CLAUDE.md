...
- Festgeld deposits are separate `Account` rows (`type="festgeld"`) that snapshot name, rate and maturity from a `FestgeldProduct` at opening. They are never auto-paid: the kid collects via `collect_festgeld` ("Abholen") after maturity. Products can be deleted freely (deposits reference nothing); deactivating just hides one from kids. Collecting is deliberately not gated on the per-kid `festgeld_enabled` switch.
