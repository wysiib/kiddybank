...
- **Interest and pocket money are lazy.** There is no scheduler. `ensure_up_to_date()` (interest accrual + due `RecurringRule`s) must run before *any* balance change on an account, so interest is computed on the balance that actually held; `post()` guarantees it. The `kid` and `parent` dependencies in `main.py` call `catch_up_user` on every request. Interest accrues into `Account.interest_accrued` (remainder carried between payouts, unit 1/(10000·365) cent; payouts round to the nearest cent) and is booked every `Account.payout_days` (7/30/365, `PAYOUT_PERIODS`, chosen per kid).
