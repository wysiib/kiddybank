...
**Auth** — profile picker + 4-digit PIN (scrypt), signed session cookie holding `uid`. Five wrong PINs in a row lock a user for five minutes (`auth.pin_failed`); this counts login attempts against the profile and cash-desk attempts against the kid asking, so a kid cannot guess a parent PIN at `/bar/*`. Roles are `parent` / `child`: `Depends(kid)` redirects parents to `/eltern`, `Depends(parent)` returns 403 for kids. Kid routes must scope every account lookup to `user.id` (see `statement`, `_resolve`, `festgeld_collect`).
